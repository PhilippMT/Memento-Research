...
- ✅ **Remote-execution tasks routed to a runner**: any task that launches code on remote infra (training, sweep, eval on a cluster) has an assignee whose skill column includes `experiment_runner` (which auto-loads the `autoresearch` runbook). Purely-local tasks (notebook analysis, write-up) are exempt.
