    ...
    def _dispatch_critic(self, producer_result: str):
        ...
        if stage["id"] == 3:
            desc += (
                ...
                "`### h… —` items grounded in real claim citations. This format "
                "is correct and intentional — it is NOT hand-written prose.\n"
                "PASS when: a topic heading is present, there is a "
                "`# Selected Hypotheses` section, and it contains at least one "
                "hypothesis (`### h…`) citing claim IDs. Do NOT require the "
